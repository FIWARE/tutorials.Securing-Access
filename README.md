...
This tutorial secures access to a FIWARE application using the entities created
in the
...
roles and permissions within the
[previous tutorial](https://github.com/Fiware/tutorials.Roles-Permissions), and
now need to programmatically enforce this policy by adding in a simple Policy
...
and integrates the use of the [Keyrock](https://fiware-idm.readthedocs.io/en/latest/)
...
Management Frontend** can be found in the `tutorial` section of the associated
`docker-compose.yml` file - only the relevant variables are shown below:
...
The [User Gredentials](https://tools.ietf.org/html/rfc6749#section-1.3.3) grant
...
details including access permissions by adding additional parameters to a
`/user` GET request
